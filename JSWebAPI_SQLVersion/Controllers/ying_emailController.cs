...
{
    public class ying_emailController : ApiController
    {

        //API: send password to users
        [HttpPost]
        [ActionName("SendPassword")]
        public int SendPassword([FromBody] ying_email myemail)
        {
            ...
            if (count >= 1)
            {
                ...
                sendemail_status = Sendemail(myemail.email_to, "Your Password for YING App", emailcontent);
               if (sendemail_status == 20)
                {
                    ...
                    sqlCmd.CommandText = "INSERT INTO ying_email_send_history (userid,email_to,content,send_date,send_from) Values (@userid,@email_to,'"+emailcontent+"',NOW(),'ADMIN')";
                    sqlCmd.CommandText = "INSERT INTO ying_email_send_history (userid,email_to,content,send_date,send_from) Values (@userid,@email_to,'" + emailcontent+"',NOW(),'ADMIN')";
        ...
        public int SendPasswordJustCheckemail([FromBody] ying_emailaddress myemail)
        {
            ...
            if (count >= 1)
            {
                ...
                if (sendemail_status == 20)
                {
                    ...
                    MySqlCommand sqlCmd = new MySqlCommand();
                    sqlCmd.CommandType = CommandType.Text;
                    sqlCmd.CommandText = "INSERT INTO ying_email_send_history (userid,email_to,content,send_date,send_from)  select userid,@email_to,'" + emailcontent + "',NOW(),'ADMIN' from users where email=@email_to ";
                    sqlCmd.Connection = myConnection;
        ...
        public int SendPasswordJustmail(string emailto)
        {
            ...
            if (count >= 1)
            {
                ...
                int sendemail_status = 0;
                sendemail_status = Sendemail(emailto, "Your Password for YING App (Do not reply)", emailcontent);
                if (sendemail_status == 20)
                {
        ...
        public int checkuserexistbyusername(string username)
        {
            ...
            string sql = "Select *  from ying_users where username='" + username + "' ";
        ...
        public int checkuserexistbyemail(string email)
        {
            ...
            string sql = "Select *  from ying_users where email='" + email + "' ";
            string connStr = ConfigurationManager.ConnectionStrings["apidb"].ConnectionString;
            MySqlConnection connsql = new MySqlConnection(connStr);
            if (connsql.State.ToString() == "Closed") connsql.Open();
            MySqlCommand Cmd = new MySqlCommand(sql, connsql);
        ...
        public string getpasswordbyemail(string email)
        {
            ...
            MySqlCommand sqlCmd = new MySqlCommand();
            sqlCmd.CommandType = CommandType.Text;
            sqlCmd.CommandText =  "select concat(concat(concat('Hi ', ' ', username), '! Your password is: ', password), '. Please try again!', ' Thank you!') from ying_users  where email = '" + email + "' ";
            sqlCmd.Connection = myConnection;
            myConnection.Open();
